...
Resultat uninominal_2nd_Tours(ListElect* lstElecteurs, Candidat** finalistes,FILE* logfile){
 ...
 void unTour(char* fichier,FILE *logfile){
    ...
    printf("************************************************************************************************");
    printf("\nVote uninominal un tour : Nombre d'électeurs : %d , Nombre de candidats : %d\n",lstElect->size,lstCand->size);
 ...
 void deuxTours(FILE* logfile,char* fichier){
    ...
    float nbVotesTotaux = lstElect->size;
    printf("\n************************************************************************************************");
    printf("\nVote uninominal deux tours : Nombre d'électeurs : %d , Nombre de candidats : %d\n",lstElect->size,lstCand->size);
    Candidat** sec = finalistes_uninominal_deux_tours(lstElect,lstCand,logfile);

    if(sec[1]!=NULL){
        printf("\nLes finalistes sont : \n  -%s votes : %f%%\n  -%s votes : %f%%\n",sec[0]->nom,(sec[0]->vote/nbVotesTotaux)*100,sec[1]->nom,(sec[1]->vote/nbVotesTotaux)*100);
        ...
        printf("\nLe gagnant du vote uninominal 2nd tour est : %s avec %f %% votes\n",gagnant.candidat->nom,gagnant.nb);
        printf("************************************************************************************************\n");
