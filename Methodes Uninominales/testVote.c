...
Resultat uninominal_2nd_Tours(ListElect* lstElecteurs, Candidat** finalistes,FILE* logfile){
 ...
 void unTour(ListElect* test,ListCand* lstCand,FILE *logfile){
 ...
 ListCand* bis(Candidat** tabCand,ListCand* retour){
    char* nom1 = strdup(tabCand[0]->nom);
    char* prenom1= strdup(tabCand[0]->prenom);
    int age1 = tabCand[0]->age;



    char* nom2 = strdup(tabCand[1]->nom);
    char* prenom2 = strdup(tabCand[1]->prenom);
    int age2 = tabCand[0]->age;

    Candidat* c1 = create_candidat(nom1,prenom1,age1);
    c1->vote = tabCand[0]->vote;
    Candidat* c2 = create_candidat(nom2,prenom2,age2);
    c2->vote = tabCand[1]->vote;
    listCand_add(retour,c1);
    listCand_add(retour,c2);

    return retour;
 }

void createFinal(Candidat** tab,ListCand* lst){
    char* nom = tab[0]->nom;
    char* prenom = tab[0]->prenom;
    int age = tab[0]->age;

    char* nom1 = tab[1]->nom;
    char* prenom1 = tab[1]->prenom;
    int age1 = tab[0]->age;

    Candidat* c1 = create_candidat(nom,prenom,age);
    Candidat* c2 = create_candidat(nom1,prenom1,age1);
    listCand_add(lst,c1);
    listCand_add(lst,c2);
...
void deuxTours(ListElect *test ,ListCand* lstCand,FILE* logfile,char* fichier){
    ...
    if(sec[1]!=NULL){
        ...
        ListCand* deuxTours = listCand_create();
        listCand_add(deuxTours,sec[0]);
        listCand_add(deuxTours,sec[1]);
        ListElect* electeurs2tours = listElect_create();
        int tabF[deuxTours->size];
        getElecteur2ndTour(electeurs2tours,deuxTours,fichier,tabF);
        initVote(electeurs2tours,deuxTours,logfile);
        Resultat gagnant = uninominal_2nd_Tours(electeurs2tours,sec,logfile);
        printf("Le gagnant du vote uninominal 2nd tour est : %s avec %f %% votes\n",gagnant.candidat->nom,gagnant.nb);
        listElect_delete(electeurs2tours);
...
int main(int argc,char** argv){
    ...
    unTour(lstElect,lstCand,logfile);
    deuxTours(lstElect,lstCand,logfile,argv[1]);
